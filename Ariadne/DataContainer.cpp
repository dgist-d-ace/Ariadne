...
map<string, int> DataContainer::getValue_yolo_missions() {	mtx_yolo_mission_map.lock(); map<string, int> temp = m_yolo_mission_map; mtx_yolo_mission_map.unlock(); }
...
QImage DataContainer::getValue_ui_pathmap() { mtx_ui_pathmap.lock(); QImage temp = m_ui_pathmap; mtx_ui_pathmap.unlock(); return temp; }
QImage DataContainer::getValue_ui_scnn() { mtx_ui_scnn.lock(); QImage temp = m_ui_scnn; mtx_ui_scnn.unlock(); return temp; }
QImage DataContainer::getValue_ui_yolo() { mtx_ui_yolo.lock(); QImage temp = m_ui_yolo; mtx_ui_yolo.unlock(); return temp; }

void DataContainer::setValue_ui_pathmap(QImage img) { mtx_ui_pathmap.lock(); m_ui_pathmap = img; mtx_ui_pathmap.unlock(); }
void DataContainer::setValue_ui_scnn(QImage img) { mtx_ui_scnn.lock(); m_ui_scnn = img; mtx_ui_scnn.unlock(); }
void DataContainer::setValue_ui_yolo(QImage img) { mtx_ui_yolo.lock(); m_ui_yolo = img; mtx_ui_yolo.unlock(); }